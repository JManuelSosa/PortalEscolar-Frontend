...
export const useGroupsByCareer = (careerId) => {

    const schoolId = useSchoolStore((state) => state.currentSchoolId);

    return useQuery({
        queryKey: queryKeys.groupsByCareer(schoolId, careerId),
        queryFn: () => queryGetService.getGroupsByCareer(careerId),
        staleTime: 15 * (60 * 1000), 
        gcTime: 20 * (60 * 1000),
        enabled: !!schoolId && !!careerId
    });
}